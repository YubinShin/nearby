...
| `lat`, `lon` | double | – | 기준 좌표. 두 값이 모두 있을 때만 위치 기반 기능이 활성화됩니다. 범위를 벗어나면 clamp하지 않고 400을 반환합니다. |
...
좌표는 예외입니다. `lat`을 90으로 보정하면 북극에서 검색하게 되므로, 조용히 틀린 결과를 내는 대신 400을 반환합니다. `NaN`과 무한대도 같습니다.
