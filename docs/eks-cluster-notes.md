...
kind 는 단일 노드라 이 질문에 답할 수 없습니다. 노드를 둘로 나눈 클러스터를 띄우고, 색인기와 질의기를 다른 노드에 놓고 지연을 다시 측정했습니다.
...
이 비대칭이 확인되면 "앱 분리는 앱 CPU 경쟁만 해소한다"는 결론이 데이터로 남습니다. 메모리 요청 합 ~2.8Gi · 제한 합 ~5Gi · ES 힙 1g.
...
두 노드그룹을 같은 AZ 에 두는 이유는 대조군(패스 B) 입니다. 패스를 바꾸면 ES·Qdrant·PostGIS 가 노드를 옮기는데, EBS 볼륨은 AZ 에 묶여 있어 다른 AZ 의 노드에는 붙지 않습니다. storageclass 의 `WaitForFirstConsumer`가 첫 소비자의 AZ 에 볼륨을 만들어 두므로, AZ 가 갈리면 패스 B 에서 PVC 가 영구 Pending 이 됩니다. 덤으로 AZ 간 네트워크 지연이 변수에서 빠집니다 — 색인기에서 PostGIS 로 가는 경로가 노드를 건너갑니다.
...
- anti-affinity 를 제거합니다. 패스 A 가 `required`로 고정해 두었으므로 그대로 두면 둘 중 하나가 영구 Pending 이 됩니다. `nodeSelector`로 같은 노드에 보내면서 anti-affinity 를 남겨두는 것은 모순입니다.
...
`requests.cpu: 6`은 8 vCPU 노드(할당 가능 ~7.9)의 3/4 입니다. Deployment 기본 전략은 새 파드를 먼저 띄우고 옛 파드를 내리는데, 그러면 잠깐 12 vCPU 가 필요해 새 파드가 영구 Pending 이 됩니다. 새 파드가 Ready 가 아니니 옛 파드도 내려가지 않습니다 — 쿠버네티스가 감지하지 않는 순환 대기라 기다려도 풀리지 않습니다.

2026-07-26 패스 A→B 전환에서 발생했고, 옛 파드를 직접 삭제해서 해소했습니다. 노드 절반 이상을 요청하는 파드에는 서지가 성립하지 않습니다.
...
그대로 비교하면 런타임 × 환경 × 토폴로지가 한꺼번에 바뀝니다. `cluster.yaml`이 색인기 메모리를 변경하지 않으며 세운 "한 번에 하나만 바꾼다" 원칙을 비교 기준이 위반하는 셈입니다. 그래서 대조군을 같은 클러스터 안으로 가져왔습니다(패스 B).
...
`ng-index`에 테인트를 걸면 coredns 같은 시스템 파드까지 막아 측정이 더 깨끗해집니다. 적용하지 않았습니다 — 얻는 정밀도(coredns 는 CPU 를 거의 쓰지 않습니다)보다 잃는 것이 큽니다. 톨러레이션을 하나 빠뜨리면 파드가 조용히 Pending 이 되고, 그 디버깅이 실험보다 오래 걸립니다. 배치 보장은 `nodeSelector`(`overlays/eks`) 로 충분합니다.
...
프로파일러로 커널을 확인한 값이 아니라 dtype 과 하드웨어 스펙으로 좁힌 추정입니다. 위 세 줄은 "코어 수 대 시간" 곡선이 아니라 환경별 실측 세 개로만 읽어야 합니다. 코어 수로 보간하지 않습니다.
