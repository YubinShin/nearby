...
1. 순차 지연. 파싱(2~3s) → 검색(~10ms) → 생성(2~3s) = 왕복 약 5~6초. LLM 왕복이 검색의 200배(ADR 0014)이고 이제 두 번입니다. 선택지: (a) 생성을 opt-in(`?answer=true`)으로 두고 기본은 0014 그대로, (b) 파싱 생략 후 생성 단일 호출로 통합(질의 이해를 생성 프롬프트에 흡수), (c) SSE 스트리밍으로 체감 지연 완화(ADR 0006 이 SSE·백프레셔를 근거로 리액티브를 선택). TBD.
...
- 골든셋 라벨이 채워질 때 — groundedness·정확도 수치를 처음으로 주장. 그전까지는 스팟 실험만.
