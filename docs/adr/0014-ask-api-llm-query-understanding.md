...
~~질의 이해가 검색 품질을 높였는지 판단할 데이터가 없습니다.~~ **2026-08-05** 측정했습니다. `scripts/eval/golden_set.yaml`을 라벨링하고(25질의·정답 434건) 같은 라벨로 원문 `q`와 `ask-api`가 만든 합성 `q`를 비교했습니다. k=10, 픽스처 모드.

| 경로 | precision@10 | recall@10 | MRR | nDCG@10 |
|---|---:|---:|---:|---:|
| 하이브리드 (원문 `q`) | 0.86 | 0.54 | 0.87 | 0.86 |
| 하이브리드 (합성 `q`) | 0.91 | 0.54 | 0.96 | **0.89** |

이유는 재현율이 아니라 순위입니다 — recall은 같고 MRR이 0.87 → 0.96으로 올랐습니다. `category_hint`가 키워드 채널에 잡을 말을 만듭니다.

| 질의 | nDCG | 내역 |
|---|---:|---|
| 조용히 공부할 곳 | 0.00 → 1.00 | `스터디카페`로 독서실을 잡음. 벡터 문턱 0.84가 자르던 것 |
| 차 고치는 곳 | 0.00 → 0.68 | `자동차 정비`로 정비소를 잡음. 같은 원인 |
| 세탁소 | 1.00 → 0.00 | 오파싱(`세타포`). 이 한 건이 평균을 0.04 깎음 |
| 배달 되는 치킨집 | 0.77 → 0.25 | `치킨집`으로 줄었는데 결과가 1건뿐 |

`세탁소`를 빼면 nDCG 0.85 → 0.93, MRR 0.87 → 1.00입니다. 손실은 품질 저하가 아니라 오파싱과 어휘 불일치에서 나옵니다.
...
남은 것은 축을 인지했는지를 보는 검증입니다. 검색 결과 건수가 아니라 응답의 `unsupported`에 `평점`·`영업시간`이 들어 있는지 봐야 합니다. 그 계약 테스트는 아직 없습니다.
