...
~~질의 이해가 검색 품질을 높였는지 판단할 데이터가 없습니다.~~ **2026-08-05 측정했습니다.**
`scripts/eval/golden_set.yaml` 을 라벨링하고(25질의·정답 434건) 같은 라벨로 원문 `q` 와
`ask-api` 가 만든 합성 `q` 를 비교했습니다. k=10, 픽스처 모드.
...
**질의 이해가 이깁니다.** 이유는 재현율이 아니라 순위입니다 — recall 은 같고 MRR 이 0.87 → 0.96 으로
올랐습니다. `category_hint` 가 키워드 채널에 잡을 말을 만들어 줍니다.

| 질의 | nDCG | 무엇이 일어났나 |
|---|---:|---|
| 조용히 공부할 곳 | 0.00 → 1.00 | `스터디카페` 로 독서실을 잡음. 벡터 문턱 0.84 가 자르던 것 |
| 차 고치는 곳 | 0.00 → 0.68 | `자동차 정비` 로 정비소를 잡음. 같은 원인 |
| 세탁소 | 1.00 → 0.00 | 오파싱(`세타포`). 이 한 건이 평균을 0.04 깎음 |
| 배달 되는 치킨집 | 0.77 → 0.25 | `치킨집` 으로 줄었는데 결과가 1건뿐 |

`세탁소` 를 빼면 nDCG 0.85 → 0.93, MRR 0.87 → 1.00 입니다. **손실은 품질 저하가 아니라 오파싱과
어휘 불일치에서 나옵니다.**

후보 풀은 원문 `q` 와 합성 `q` 양쪽의 결과를 합쳐 만들었습니다. **비교할 설정이 질의 문자열을
바꾸면 그 설정의 결과도 풀에 있어야 합니다.** 한쪽 풀로만 채점하면 다른 쪽이 데려온 정답이 라벨에
없어 맞아도 오답이 됩니다 — `평점 4.5 이상 카페` 와 `카페` 의 정답 목록은 겹침이 0 입니다.

~~`category_hint` 가 키워드 채널의 `AND` 조건을 좁혀 0건을 늘릴 가능성은 **미실측**입니다.~~
**2026-08-05 측정. 순효과는 이득입니다** (위 표). 다만 좁아지는 사례는 실재합니다 —
`배달 되는 치킨집` 은 합성 `q` 가 `치킨집` 으로 줄면서 결과가 10건에서 1건이 됐습니다.
코퍼스 어휘는 `치킨` 인데 `치킨집` 을 만들어 붙인 경우로, `AND` 확장이 아니라 **어휘 불일치**입니다.
Platform gap ①(어휘 목록 경로)이 있으면 `category_hint` 를 코퍼스 어휘로 강제할 수 있습니다.
