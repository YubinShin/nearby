...
노드 분리가 질의 지연에 주는 영향을 측정하기 위한 실험 클러스터입니다. 노드그룹 2개짜리 EKS 를 띄우고, 색인기와 질의기를 분리한 배치와 합친 배치에서 각각 지연을 측정합니다.

> **실험용 클러스터입니다.** 측정이 끝나면 `./deploy/eks/down.sh` 를 실행합니다. 컨트롤 플레인은 노드를 0으로 줄여도 시간당 요금이 계속 발생합니다.
