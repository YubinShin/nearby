...
LLM 에게 답변을 요구하는 것이 아닌, 검색 요청만 구조화합니다.
...
| 필드 | 채우는 것 |
|---|---|
| `expected_places` | 이 질의에 나와야 하는 `place_id` 목록. 순서는 보지 않는다 |
| `expect_empty` | 결과가 0건이어야 맞는 질의면 `true` |

트랩 5개의 `expect_empty` 만 `true` 로 채워져 있습니다.

1. `./deploy/up.sh gangnam` · `search-api` 기동
2. `curl -G localhost:8080/v1/hsearch --data-urlencode "q=<질의>" -d size=20`
3. 맞는 결과의 `placeId` 를 `expected_places` 에 적는다. 결과에 없더라도 정답이라고 판단하는 `placeId` 가 있으면 함께 적는다. (recall 측정 시 필요)
4. 하나도 맞는 것이 없어야 정상인 질의면 `expect_empty: true`
