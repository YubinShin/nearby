...
class AnalyzerFingerprintTest {
	...
	fun `the same token stream always digests to the same value`() {
		assertEquals(AnalyzerFingerprint.digest(tokens), AnalyzerFingerprint.digest(tokens))
	}

	@Test
	fun `a dictionary change that splits differently changes the digest`() {
		val split = listOf("놓:0:1", "ㄴ:1:2", "현:2:3", "투썸플레이스:4:10")
		assertNotEquals(AnalyzerFingerprint.digest(tokens), AnalyzerFingerprint.digest(split))
	}

	@Test
	fun `the same terms at different offsets digest differently`() {
		val shifted = listOf("논현:0:2", "동:2:3", "투썸플레이스:5:11")
		assertNotEquals(AnalyzerFingerprint.digest(tokens), AnalyzerFingerprint.digest(shifted))
	}

	@Test
	fun `a dropped token changes the digest`() {
		assertNotEquals(AnalyzerFingerprint.digest(tokens), AnalyzerFingerprint.digest(tokens.dropLast(1)))
	}

	@Test
	fun `the digest is twelve hex characters`() {
		val digest = AnalyzerFingerprint.digest(tokens)
	...
	fun `an empty token stream still digests`() {
		assertEquals(12, AnalyzerFingerprint.digest(emptyList()).length)
	}
}