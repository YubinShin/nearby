...
검색 품질을 평가하기 위한 골든셋과 채점 도구입니다. `golden_set.yaml`에는 **25개 질의**, **434개 정답(place_id)** 이 있습니다. (Labeling: **2026-08-05**)
...
속성을 올바르게 인식했는지는 답변의 `unsupported` 필드로 별도 검증해야 하며, 해당 계약 테스트는 아직 구현되지 않았습니다.
...
| Search Path | Precision@10 | Recall@10 | MRR | nDCG@10 |
| --- | ---: | ---: | ---: | ---: |
| Keyword | 0.60 | 0.29 | 0.64 | 0.53 |
| Vector | 0.76 | 0.43 | 0.79 | 0.72 |
| Hybrid (`q`) | 0.86 | 0.54 | 0.87 | 0.86 |
| **Hybrid (`ask`-generated `q`)** | **0.91** | **0.54** | **0.96** | **0.89** |

평균은 **25개 질의 전체**를 대상으로 계산하므로, 키워드 검색이 결과를 전혀 반환하지 못한 9개 질의도 0점으로 포함됩니다. 원본 측정 결과는 `scores/2026-08-05-*.json`에 있습니다.
...
| 회 먹을 데 | +0.35 | 질의 이해로 순위 개선 |
| 세탁소 | 1.00 → 0.00 | LLM이 `세타포`로 오인식 |
| 배달 되는 치킨집 | 0.77 → 0.25 | `치킨집`으로 단순화되며 결과가 1건만 남음 |

`세탁소` 오인식 하나가 평균 nDCG를 약 **0.04** 낮춥니다. 해당 질의를 제외하면 nDCG **0.85 → 0.93**, MRR **0.87 → 1.00** 입니다.
