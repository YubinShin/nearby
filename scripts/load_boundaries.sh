...
docker exec -i psp-postgis psql -U place -d "$DB" -q < deploy/postgis/boundaries.sql

echo "▶ 2/3  GeoJSON → SQL (서울만)"
python3 scripts/boundaries_to_sql.py > data/adm_dong.sql

echo "▶ 3/3  적재"
docker exec -i psp-postgis psql -U place -d "$DB" -q < data/adm_dong.sql

echo -n "✔ 적재 건수: "
docker exec -i psp-postgis psql -U place -d "$DB" -tAc "SELECT count(*) FROM public.adm_dong;"
echo -n "  강남구:    "
docker exec -i psp-postgis psql -U place -d "$DB" -tAc \
