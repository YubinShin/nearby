...
docker exec -i psp-postgis psql -U place -d place < deploy/postgis/schema.sql

echo "▶ 3/3  적재 (COPY)"
cat data/gangnam_place.csv | docker exec -i psp-postgis psql -U place -d place \
...
echo -n "✔ 적재 건수: "
docker exec -i psp-postgis psql -U place -d place -tAc "SELECT count(*) FROM public.place;"
